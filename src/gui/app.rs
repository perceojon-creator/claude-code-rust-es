...
impl ClaudeCodeApp {
    ...
    fn handle_send_message(&mut self, messages: Vec<crate::api::ChatMessage>) {
        let api_client = match self.api_client.clone() {
            Some(client) => client,
            None => {
                self.show_status("Cliente API no inicializado");
        ...
        tokio::spawn(async move {
            if api_client.get_api_key().is_none() {
                tx.send(GuiMessage::ApiError {
                    error: "Clave de API no configurada. Configúrala en ajustes.".to_string(),
                })
                .await
                .ok();
                return;
                ...
                Ok(response) => {
                    ...
                    while let Some(chunk) = stream.next().await {
                        match chunk {
                            ...
                            Err(e) => {
                                tx.send(GuiMessage::ApiError {
                                    error: format!("Error de transmisión: {}", e),
                                })
                                .await
                                .ok();
                                return;
